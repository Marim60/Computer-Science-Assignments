...
public class Index5 {
    ...
    //-----------------------------------------------
    // build the inverted index from disk not from the internet
...
//      10 return answer
    ...
    public String find_24_01(String phrase) { // any number of terms non-optimized search
        ...
        // now if word is not in the hash table will not crash! by checking first
        // if the word exist in the index, if any word not exist this mean no document contain
        ...
        for (String w : words) {
            ...
            if (index.containsKey(word)) { // word exist in the index
                ...
                // if this is not the first word, we will intersect the current posting list
                else {
